...
# Set process name to 'claude' for cleaner process listings
$0 = 'claude';
...
if(!-d ".git"){
    # Create an empty .git directory to satisfy Claude's checks
    mkdir(".git")
        or die "Failed to create directory .git: $!";
}
...
if($< == 0 and length($ENV{UID}//"")){
    ...
    # Change ownership of Claude's data directory to the target UID (directory only, not contents)
    chown($target_uid, 1000, "/workspace/.claude")
        or warn "Warning: could not chown /workspace/.claude: $!";
...
if($< == 0){
    # Change ownership to 1000:1000 (directory only, not contents)
    chown(1000, 1000, "/workspace/.claude")
        or warn "Warning: could not chown /workspace/.claude: $!";
...
# Execute the actual Claude CLI with all provided arguments
exec("/home/node/.local/bin/claude", @ARGV)
